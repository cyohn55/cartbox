...
/**
 * Full-screen session for the OpenTyrian (Tyrian 2000) catalog title inside the
 * handheld.
 *
 * OpenTyrian2000 is a whole SDL2 application, not a Cartbox Game ABI module, so —
 * like SuperTux and ScummVM — it runs in a same-origin iframe
 * (public/opentyrian/cartbox-boot.html) that owns its canvas, main loop and audio.
 * It presents identically to the other iframe players (same stage, loading copy,
 * SELECT = EJECT hint) so the handheld feels like one console.
 *
 * Because Tyrian is keyboard-driven, the integration mirrors SuperTuxPlayer: the
 * shell forwards each button as a synthetic KeyboardEvent on `window`, and this
 * component re-dispatches the Tyrian key (see the pure opentyrianRuntime map) into
 * the iframe. The one difference from SuperTux is the dispatch target: SDL2's
 * Emscripten backend registers its keyboard callbacks on the window/document
 * rather than the canvas, so events are dispatched on the iframe document (they
 * bubble to window). Real taps still pass through.
 */
...
import { opentyrianKeyForControl } from "@/lib/opentyrianRuntime";
...
/** Dispatches synthetic keyboard events into the OpenTyrian iframe. */
class IframeKeyboard {
  ...
  key(code: string, down: boolean): void {
    ...
    // SDL2's Emscripten backend keys off `code`; `key`/`keyCode` are filled so
    // handlers that read them still see a coherent event. Dispatched with bubbling
    // so it reaches the window listener SDL2 installs by default.
    target.dispatchEvent(new win.KeyboardEvent(down ? "keydown" : "keyup", { code, bubbles: true }));
  }
}

export function OpenTyrianPlayer({ cart, onExit }: { cart: PlayingCart; onExit: () => void }) {
  const frameRef = useRef<HTMLIFrameElement>(null);
  const inputRef = useRef<IframeKeyboard | null>(null);
  const [status, setStatus] = useState<"loading" | "ready" | "error">("loading");
  const [progress, setProgress] = useState<number | null>(null);

  const src = withBasePath("/opentyrian/cartbox-boot.html");
  ...
  useEffect(() => {
    const onMessage = (event: MessageEvent) => {
      ...
      if (data?.source !== "cartbox-opentyrian") return;
  ...
  // Translate the shell's forwarded buttons into Tyrian keys and dispatch them
  ...
  useEffect(() => {
    const route = (code: string, down: boolean): void => {
      ...
      const key = opentyrianKeyForControl(control);
