...
// 1. Each region drives the right shell variable, with derived darker stops.
{
  ...
  assert.equal(vars["--hh-dpad-a"], "#57d18d", "d-pad ring -> dpad base");
  assert.equal(vars["--hh-joy-a"], "#57d18d", "d-pad ring -> joystick base");
  assert.equal(vars["--hh-control-a"], "#3a3d42", "shoulder -> control base");
  assert.equal(vars["--hh-face-ink"], "#fad937", "button letters -> face ink");
  // All four face buttons take the diamond colour.
...
{
  const vars = handheldConsoleVariables({ ...scheme, face: "not-a-color", dpadRing: "#GGG" });
