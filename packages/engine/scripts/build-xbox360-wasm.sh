#!/usr/bin/env bash
# Build the Cartbox Xbox 360 core: a 320x240, 8bpp, 8-channel build of the same
...
# all 3.1.x; a Xbox 360 core must match so it loads everywhere they do.
set -euo pipefail

ENGINE_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
TIC80_DIR="${ENGINE_DIR}/tic80"
BUILD_DIR="${ENGINE_DIR}/build-wasm-xbox360"
DIST_DIR="${ENGINE_DIR}/dist/xbox360"
...
XBOX360_WIDTH=1280
XBOX360_HEIGHT=720
XBOX360_FULLWIDTH_BITS=11
XBOX360_PALETTE_BPP=8
XBOX360_SOUND_CHANNELS=8
XBOX360_VRAM_SIZE=$((2 * 1024 * 1024))   # 2 MiB: 1280x720 8bpp screen is ~900KB
XBOX360_RAM_SIZE=$((8 * 1024 * 1024))    # 8 MiB: vram + map + tiles + overhead at 720p
XBOX360_MAP_MAX_SIZE=$((2 * 1024 * 1024)) # sanity bound for the 720p map

XBOX360_DEFINES="-DTIC80_WIDTH=${XBOX360_WIDTH} -DTIC80_HEIGHT=${XBOX360_HEIGHT}"
XBOX360_DEFINES+=" -DTIC80_FULLWIDTH_BITS=${XBOX360_FULLWIDTH_BITS}"
XBOX360_DEFINES+=" -DTIC_PALETTE_BPP=${XBOX360_PALETTE_BPP}"
XBOX360_DEFINES+=" -DTIC_SOUND_CHANNELS=${XBOX360_SOUND_CHANNELS}"
XBOX360_DEFINES+=" -DTIC_VRAM_SIZE=${XBOX360_VRAM_SIZE} -DTIC_RAM_SIZE=${XBOX360_RAM_SIZE}"
XBOX360_DEFINES+=" -DTIC_MAP_MAX_SIZE=${XBOX360_MAP_MAX_SIZE}"
...
# --- 1. Build the TIC-80 core as a static library (xbox360 config) ---------------
# The xbox360 -D defines reach every core translation unit via CMAKE_C_FLAGS so the
# memory-map structs are laid out at the pro sizes.
emcmake cmake -S "${TIC80_DIR}" -B "${BUILD_DIR}" \
  -DCMAKE_BUILD_TYPE=MinSizeRel \
  -DCMAKE_POLICY_VERSION_MINIMUM=3.5 \
  -DCMAKE_C_FLAGS="${XBOX360_DEFINES}" \
  -DBUILD_SDL=OFF \
  -DBUILD_EDITORS=OFF \
  -DBUILD_TOOLS=OFF \
  -DBUILD_PLAYER=OFF \
...
# --- 2. Compile + link the shim into an ES module (xbox360 config) ---------------
# Same shim.c as classic; it reads dimensions from the (now xbox360) macros. The xbox360
...
emcc -O3 \
  ...
  -sEXPORT_NAME=Xbox360Engine

echo "Built ${DIST_DIR}/engine.js (+ engine.wasm) — xbox360 core, ${XBOX360_WIDTH}x${XBOX360_HEIGHT}."
